...
class MeetingDB(Firestore_Base_DB):
    ...
    def check_overlap(self, start_data, end_data, student, tutor, UID):
        ...
        start_time = util.translate_time(start_data)
        end_time = util.translate_time(end_data)
    ...
    def get_parsed_timestamp_list(self,role,UID):
        ...
        parsed_list = []
        for meet in meetings:
            meeting_date_start = util.translate_time(meet.get("startDateTime"))
            meeting_date_end = util.translate_time(meet.get("endDateTime"))
    ...
    def overleap_check(self, tutor_id, lesson_id, start, end):
        start = util.translate_time(start)
        end = util.translate_time(end)
        meetings = self.get_meeting_tutorid_lessonid(tutor_id, lesson_id)
        for meeting in meetings:
            m_start = util.translate_time(meeting['startDateTime'])
            m_end = util.translate_time(meeting['endDateTime'])
            if not (m_start < m_end < start < end or start < end < m_start < m_end):
                return False
        return True
